...
// List blocks by date
// LitecoinHaus: custom method (lists blockhash + number of transactions from the past 48 hours)
BlockController.prototype.list = function(req, res) {
  ...
  var gte = Math.round((new Date()).getTime() / 1000);

  //pagination
  var lte = gte + 86400; // unixtimestamp in 24 hours time
  var prev = this.formatTimestamp(new Date((gte - 86400) * 1000)); // yesterday YYYY-MM-DD
  var next = lte ? this.formatTimestamp(new Date(lte * 1000)) : null; // tomorrow YYYY-MM-DD
  var limit = BLOCK_LIMIT; // 1500
  var more = false;
